...
class QuantumDiscordBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        logger.info("\U0001F916 **SETTING UP BOT COMMANDS**")
        ...
        # Start keep-alive/heartbeat task
        self.heartbeat_task = self.loop.create_task(self.keep_alive_heartbeat())

    async def keep_alive_heartbeat(self):
        """Background task to keep Discord connection alive and log heartbeats."""
        interval = getattr(config, "HEARTBEAT_INTERVAL", 60)
        channel_id = getattr(config, "TARGET_CHANNEL_ID", None)
        log_path = os.path.join(os.path.dirname(__file__), "../live_development.log")
        while True:
            try:
                ...
                if channel and isinstance(channel, discord.TextChannel):
                    # Send an invisible character as a lightweight keep-alive
                    await channel.send("\u200e")
                    log_msg = f"[{now}] [HEARTBEAT] Sent invisible keep-alive to channel {channel_id}"
                elif self.user is not None:
                    user = await self.fetch_user(self.user.id)
                    log_msg = f"[{now}] [HEARTBEAT] Fetched bot user info: {user}"
                else:
                    log_msg = f"[{now}] [HEARTBEAT] Bot user not available for fetch."
                # Write to live_development.log
                with open(log_path, "a", encoding="utf-8") as f:
                    f.write(log_msg + "\n")
                logger.info(log_msg)
            except Exception as e:
                err_msg = f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [HEARTBEAT ERROR] {e}\n{traceback.format_exc()}"
                with open(log_path, "a", encoding="utf-8") as f:
                    f.write(err_msg + "\n")
    ...
    async def on_disconnect(self):
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        log_path = os.path.join(os.path.dirname(__file__), "../live_development.log")
        msg = f"[{now}] [DISCONNECT] Bot disconnected from Discord."
        with open(log_path, "a", encoding="utf-8") as f:
            f.write(msg + "\n")
        logger.warning(msg)
    ...
    async def on_resumed(self):
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        log_path = os.path.join(os.path.dirname(__file__), "../live_development.log")
        msg = f"[{now}] [RESUME] Bot resumed connection to Discord."
        with open(log_path, "a", encoding="utf-8") as f:
            f.write(msg + "\n")
        logger.info(msg)
